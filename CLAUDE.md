...
- Native: `ios/` + `macos/` (xcodegen, shared ContentView.swift, URLSession straight to PostgREST/GoTrue for email/password/Apple; supabase-swift added just for the Google/GitHub/X OAuth browser flow (PKCE), `co-stanza://` registered as the redirect scheme and in the spark allow-list), `watchos/` (xcodegen, own ContentView.swift, read-only feed, no sign-in), `kmp/` (Ktor client in shared, Compose in composeApp; CI builds msi/deb/apk, no JDK on this Mac)
